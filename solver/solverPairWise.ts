...
function formatResult(x: Result): DisplayResult {
  const path: DisplayResult['path'] = [
    { value: x.expression[0].toString(), partialOutput: x.outputs[0] },
  ]
  for (let i = 1; i < x.expression.length; i += 2) {
    const value = `${x.expression[i]}${x.expression[i + 1]}`
    path.push({ value, partialOutput: x.outputs[i + 1] })
  }
  const ret: DisplayResult = {
    output: x.outputs[x.outputs.length - 1],
    distance: x.distance,
    path,
    resultId: x.expression.join(''),
  }
  return ret
}
